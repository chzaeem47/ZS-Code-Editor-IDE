...
const Navbar = () => {
    ...
    const ProjectRow = ({ project,selectable = false }) => (
        <div onClick={() => !selectable && handleOpenProject(project)} className={`flex items-center gap-2 rounded-xl px-2.5 py-2 transition-all duration-200 ${selectable ? "" : "cursor-pointer"} ${isDark ? "hover:bg-white/[0.08]" : "hover:bg-black/[0.05]"}`}>
            {selectable && (
            ...
            <FaFolder className={`shrink-0 text-[15px] ${isDark ? "text-cyan-400" : "text-[#1227b2]"}`}/>
            <div className="min-w-0 flex-1">
                <p className={`truncate font-serif text-xs font-semibold ${isDark ? "text-white" : "text-slate-800"}`}>{project.name}</p>
                ...
                <div className={`mx-auto flex h-[50px] max-w-[1550px] items-center justify-between border-b px-4 transition-all duration-500 sm:px-5 ${isDark ? "border-white/10 bg-[#414040]" : "border-black/10 bg-white/70"}`}>
                    <div className="flex items-center gap-2">
                        ...
                        <div ref={homeMenuRef} className="relative">
                            ...
                            {homeOpen && (
                                <div className={`absolute left-0 top-[calc(100%+8px)] w-[310px] overflow-hidden rounded-[20px] border p-2 effect-3d backdrop-blur-3xl ${isDark ? "border-white/10 bg-gradient-to-b from-[#0e163d]/98 to-[#1c0f30]/98" : "border-black/10 bg-gradient-to-b from-white/98 to-slate-100/98"}`}>
                                    <div className="flex items-center justify-between px-2 py-1">
                                        <div className="flex items-center gap-2">
                                            <FaFolder className={isDark ? "text-cyan-400" : "text-[#1227b2]"}/>
                                            <span className={`font-serif text-sm font-semibold ${isDark ? "text-white" : "text-slate-800"}`}>Projects</span>
                                        </div>
                                        <button type="button" onClick={() => setShowCreateModal(true)} className={`flex h-7 w-7 items-center justify-center rounded-full ${isDark ? "text-white/50 hover:bg-white/10 hover:text-white" : "text-slate-400 hover:bg-black/5 hover:text-slate-700"}`} title="Create Project">
                                    ...
                                    {!user ? (
                                        ...
                                        <>
                                            <div className="mb-2 flex gap-2 px-1">
                                                <button type="button" onClick={() => setShowCreateModal(true)} className="effect-3d flex h-9 flex-1 items-center justify-center gap-1 rounded-full bg-gradient-to-r from-[#0b165d] via-[#1227b2] to-[#0641e2] px-2 font-cookie text-xl tracking-widest text-white">
                                                    Create
                                                </button>
                                                <button type="button" onClick={() => setSelectionMode((previous) => !previous)} className={`effect-3d flex h-9 items-center justify-center rounded-full px-4 font-cookie text-xl tracking-widest text-white ${selectionMode ? "bg-rose-600" : "bg-gradient-to-r from-[#0b165d] via-[#1227b2] to-[#0641e2]"}`}>
                                            ...
                                            <div className="max-h-[350px] space-y-1 overflow-y-auto">
                                                {projectsLoading ? (
                                                    ...
                                                    projects.map((project) => <ProjectRow key={project._id} project={project} selectable={selectionMode}/>)
                                                ) : (
                                            ...
                                            {selectionMode && (
                                                <button type="button" onClick={handleDeleteSelected} disabled={!selectedProjects.length} className="mt-2 flex h-9 w-full items-center justify-center gap-2 rounded-full bg-gradient-to-r from-red-700 via-red-600 to-red-700 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40">
                        ...
                        <div ref={starMenuRef} className="relative">
                            ...
                            {starOpen && (
                                <div className={`absolute left-0 top-[calc(100%+8px)] w-[290px] overflow-hidden rounded-[20px] border p-2 effect-3d backdrop-blur-3xl ${isDark ? "border-white/10 bg-gradient-to-b from-[#0e163d]/98 to-[#1c0f30]/98" : "border-black/10 bg-gradient-to-b from-white/98 to-slate-100/98"}`}>
                                    <div className="flex items-center gap-2 px-2 py-1">
                                        <FaStar className="text-yellow-400"/>
                                        <span className={`font-serif text-sm font-semibold ${isDark ? "text-white" : "text-slate-800"}`}>Starred Projects</span>
                                    </div>
                                    ...
                                    {!user ? (
                                        ...
                                        <div className="max-h-[350px] overflow-y-auto">
                                            {starredLoading ? (
                                                ...
                                                <div className={`px-3 py-8 text-center text-xs ${isDark ? "text-white/40" : "text-slate-400"}`}>No starred projects yet</div>
                                            )}
                                        </div>
                                    )}
                                </div>
                    ...
                    <div className="flex items-center gap-2 sm:gap-10">
                        ...
                        {!user ? (
                            ...
                            <div ref={menuRef} className="relative">
                                ...
                                {open && (
                                    <div className={`absolute right-0 top-[calc(100%+10px)] w-[250px] overflow-hidden rounded-xl p-2 border effect-less ${isDark ? "border-white/15 bg-[#393636]" : "border-indigo-100 bg-gradient-to-b from-[#f8fafc]/95 to-[#f1f5f9]/95"}`}>
                                        <div className={`rounded-xl border p-3.5 ${isDark ? "border-white/5 bg-gradient-to-br from-white/10 via-transparent to-transparent" : "border-indigo-200/50 bg-gradient-to-br from-indigo-500/5 via-transparent to-transparent"}`}>
                                            ...
                                            <p className={`font-cookie mt-1 text-3xl tracking-widest ${isDark ? "text-white" : "text-slate-900"}`}>{user.name}</p>
                                            <p className={`font-serif mt-0.5 truncate text-[14px] ${isDark ? "text-slate-400" : "text-slate-500"}`}>{user.email}</p>
                                        </div>
                                        <button type="button" onClick={handleLogout} className={`font-cookie mt-2 flex w-full items-center gap-2.5 rounded-[20px] px-17 py-1 text-2xl tracking-wider effect-3d transition-all duration-300 ${isDark ? "border-white/10 bg-gradient-to-r from-[#581c87]/40 to-[#1e1b4b]/40 text-purple-200 hover:from-rose-500 hover:to-rose-600 hover:text-white" : "border-rose-200/60 bg-gradient-to-r from-rose-50/50 to-slate-100/50 text-rose-600 hover:from-rose-500 hover:to-rose-600 hover:text-white"}`}>
