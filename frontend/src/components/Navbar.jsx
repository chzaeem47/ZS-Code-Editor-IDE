...
const Navbar = () => {
    ...
    const handleDeleteSelected = async () => {
        ...
        const confirmed = window.confirm(
            `Delete ${selectedProjects.length} selected project${selectedProjects.length > 1 ? "s" : ""
            }?`
        );
        ...
        try {
            ...
            if (successfulIds.length) {
                toast.success(
                    `${successfulIds.length} project${successfulIds.length > 1 ? "s" : ""
    ...
    const ProjectRow = ({
        ...
        <div
            ...
            className={`flex items-center gap-2 rounded-xl px-2.5 py-2 transition-all duration-200 ${selectable ? "" : "cursor-pointer"
                } hover:scale-[1.02]`}
        >
            {selectable && (
                <button
